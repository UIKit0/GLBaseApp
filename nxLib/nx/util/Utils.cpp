...
{
private:
    std::map<UINT, wchar_t*> m_wm;
public:
    WMHelper() : m_wm() {
        size_t i = 0;
        while (s_wms[i].name != nullptr) {
            m_wm.insert(std::make_pair<UINT, wchar_t*>(s_wms[i].id, s_wms[i].name));
            ++i;
        }
    }
    const wchar_t* Get(UINT id) const {
        if (m_wm.count(id) > 0) {
            return m_wm.at(id);
        } else {
            return nullptr;
...
void nx::util::_WMessage(std::ios_base& os, WMessageArgs message)
{
    std::wostream *pos = dynamic_cast<std::wostream*>(&os);
    if (pos) {
        (*pos) << std::hex << L"hWnd=" << message.hwnd << L",message=";
        const wchar_t *msg = s_wmHelper.Get(message.message);
        if (msg == nullptr) {
            (*pos) << L"UNKNOWN(" << message.message << L")";
        } else {
            (*pos) << msg;
        }
        (*pos) << L",wParam=" << message.wParam
               << L",lParam=" << message.lParam;
    }
}

void nx::util::_LastError( std::ios_base& os, LastErrorArgs args )
{
    std::wostream *pos = dynamic_cast<std::wostream*>(&os);
    if (pos) {
        LPVOID lpMsgBuf;
...
bool nx::util::SplitToFolderAndFilename(const std::wstring &path, std::wstring &folder, std::wstring &filename)
{
    const static std::wstring PATH_SEPARATOR(L"\\");
    ...
    filename = path.substr(pos, path.length() - pos);
    if (folder.at(0) != L'\\' ) {
        folder = PATH_SEPARATOR + folder;
    }
    return true;
}
